    ...
    if ($msg == '') {
        if (isset($_GET['id']) && $_GET['id'] != '') {
            if ($_FILES['image']['name'] != '') {
                // Here using provided image
                // Image name
                $image = rand(111111111, 999999999) . '_' . $_FILES['image']['name'];
                // Uploading the image
                move_uploaded_file($_FILES['image']['tmp_name'], PRODUCT_IMAGE_SERVER_PATH.$image);
                echo "Update 1";
                $update_sql = "update product set category_id='$categories_id',name='$name',mrp='$mrp',price='$price',quantity='$qty',short_desc='$short_desc',description='$description',meta_title='$meta_title',meta_desc='$meta_desc',meta_keyword='$meta_keyword',image='$image' where id='$id'";
            } else {
                // If image is not provided
                $update_sql = "update product set category_id='$categories_id',name='$name',mrp='$mrp',price='$price',quantity='$qty',short_desc='$short_desc',description='$description',meta_title='$meta_title',meta_desc='$meta_desc',meta_keyword='$meta_keyword' where id='$id'";
            }
            mysqli_query($con, $update_sql);
        } else {
            $image = rand(111111111, 999999999).'_'.$_FILES['image']['name'];
            move_uploaded_file($_FILES['image']['tmp_name'], PRODUCT_IMAGE_SERVER_PATH.$image);
            mysqli_query($con, "insert into product(category_id,name,mrp,price,quantity,short_desc,description,meta_title,meta_desc,meta_keyword,status,image) values('$categories_id','$name','$mrp','$price','$qty','$short_desc','$description','$meta_title','$meta_desc','$meta_keyword',1,'$image')");
        }
        header('location:product.php');
        die();
    }
